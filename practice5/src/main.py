...
def get_index_path(args, collection_name) -> str:
    index_path = f"{SAVE_FOLDER}/index_{collection_name}_"
...
def main() -> None:
    ...
    for collection_name in COLLECTION_NAME:
        # Create the path to the save index file for each collection
        index_path = get_index_path(args, collection_name)
        ...
        if is_existing_index:
            ...
            # Create the TextPreProcessor object
            ...
            doc_id = int(collection_name)
            # Check if we need to compute the indexed Collection
            if args.generate_index or not is_existing_index:
                index = Indexer()
                collection = Collection(
                    doc_id=doc_id,
                    path=os.path.join(DATA_FOLDER, collection_name + ".xml"),
                    indexer=index,
